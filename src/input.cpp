...
#define ENABLE_LEFT_BUTTON_DELAY nullptr
  ...
  abs_mouse_sync_estimate(const std::shared_ptr<input_t> &input, bool client_idle, bool sat_x, bool sat_y, float port_w, float port_h) {
    ...
    if (client_idle) {
      ...
      // The fractional residue was relative to the pre-snap estimate; drop it
      // so the primary loop recomputes a clean delta from the real position.
      input->abs_mouse.frac_x = 0.0f;
      input->abs_mouse.frac_y = 0.0f;
    }
    else {
      if (sat_x) {
        input->abs_mouse.host_x = std::clamp(real_x, 0.0f, port_w - 1.0f);
